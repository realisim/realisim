//------------------------------------------------------------------------------
//! AUTHOR:  Pierre-Olivier Beaudoin
//------------------------------------------------------------------------------

#ifndef realisim_math_intersection_hh
#define realisim_math_intersection_hh

#include "math/Primitives.h"
#include "math/Point.h"
//#include "Line3d.h"
#include "Plane.h"
#include <vector>

//------------------------------------------------------------------------------
namespace realisim
{
namespace math
{
	class Intersection2d
  {
  public:
  	Intersection2d() : mPoints(), mNormals() {;}
    Intersection2d( const Intersection2d& i ) : mPoints( i.mPoints ),
      mNormals( i.mNormals ) {}
    Intersection2d& operator=(const Intersection2d& i)
    { mPoints = i.mPoints; mNormals = i.mNormals; return *this; }
    virtual ~Intersection2d() {}
    
    virtual void add( const Intersection2d& );
    virtual void add( const Point2d&, const Vector2d& );
    virtual void clear() {mPoints.clear(); mNormals.clear();}
    virtual bool hasPoints() const { return !mPoints.empty(); }
    virtual int numberOfPoints() const { return (int)mPoints.size(); }
    virtual Point2d point(int i) const { return mPoints[i]; }
    virtual Vector2d normal(int i) const { return mNormals[i]; }
    
  protected:
    std::vector< Point2d > mPoints;
    std::vector< Vector2d > mNormals;
  };
	enum intersectionType{ itNone, itPoint, itContained };
  
//Point3d intersect( const Line3d&, const Plane& );
//Point3d intersect( const Plane&, const Line3d& );

	Intersection2d intersects( const Circle&, const LineSegment2d& );
  Intersection2d intersects( const LineSegment2d&, const Circle& );
  Intersection2d intersects( const Circle&, const Rectangle& );
  Intersection2d intersects( const Rectangle&, const Circle& );
} //math
} // end of namespace realisim
#endif // LINE_H
